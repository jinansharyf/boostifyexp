  ...
  useEffect(() => {
    (async () => {
      const ids = Array.from(
        new Set((q.data ?? []).map((o: any) => o.vendor_id).filter(Boolean)),
      ) as string[];
                ...
                <Fragment key={o.id}>
                  ...
                  >
                    ...
                    <TableCell className="max-w-[220px]">
                      <span className="line-clamp-2 whitespace-normal break-words text-sm">
                        {o.delivery_address}
                      </span>
                    </TableCell>
                    ...
                    <TableCell onClick={(e) => e.stopPropagation()}>
                      <ActionRow
                        status={o.status}
                        onAction={(next) => m.mutate({ id: o.id, status: next })}
  ...
  const rows: Array<[string, React.ReactNode]> = [
    ...
    [
      ...
      order.customer_phone ? (
        <a className="text-primary underline" href={`tel:${order.customer_phone}`}>
          {order.customer_phone}
        </a>
      ) : (
        "—"
    ...
    [
      "Delivery address",
      <span className="whitespace-pre-wrap break-words">{order.delivery_address || "—"}</span>,
    ],
    ...
    <div className="space-y-4">
      <div className="grid gap-x-6 gap-y-2 text-sm md:grid-cols-2">
        {rows.map(([label, value]) => (
          <div
            key={label}
            className="flex flex-col gap-0.5 rounded-md border border-border/60 bg-card/60 px-3 py-2"
          >
          ...
          <ol className="space-y-1 text-xs">
            {order.timeline.map((e: any, i: number) => (
              <li key={i} className="flex flex-wrap items-center gap-2">
                ...
                <span>
                  · by <strong>{e.by_name || "system"}</strong>
                </span>
              </li>
            ))}
          </ol>
