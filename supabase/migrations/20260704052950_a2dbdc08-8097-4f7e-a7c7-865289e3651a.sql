-- Boostify backend repair: missing pricing, quick replies, vendor settings requests, and tracking support.
...
-- Ensure zones exists for pricing in partially imported databases.
...
DROP TRIGGER IF EXISTS zones_updated ON public.zones;
...
CREATE POLICY vehicle_types_admin_write ON public.vehicle_types
  ...
  USING (public.is_admin(auth.uid()))
  WITH CHECK (public.is_admin(auth.uid()));
DROP TRIGGER IF EXISTS vehicle_types_updated ON public.vehicle_types;
CREATE TRIGGER vehicle_types_updated BEFORE UPDATE ON public.vehicle_types
  FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();
...
CREATE POLICY delivery_prices_admin_write ON public.delivery_prices
  ...
  USING (public.is_admin(auth.uid()))
  WITH CHECK (public.is_admin(auth.uid()));
DROP TRIGGER IF EXISTS delivery_prices_updated ON public.delivery_prices;
CREATE TRIGGER delivery_prices_updated BEFORE UPDATE ON public.delivery_prices
  FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();
